...
## Versi Saat Ini
v67 — Batch67 (Audit Gap P2 #11 stage 1/4: hardcoded strings → strings.xml di OnboardingScreen.kt, 2 file)
...
- **AUDIT GAP TRACKER (mulai Batch38)** — 20 temuan P0/P1/P2. Sumber sekarang **`AUDIT_GAP.md` di root repo** (ditanamkan permanen Batch45 — sebelumnya cuma ada sebagai upload chat sesi lama, itu sebabnya sempat jadi BLOCKER di awal sesi ini). Dikerjakan bertahap per batch (bukan 1 batch raksasa — di luar batas 3-file/batch project ini). Status:
  ...
  - ⏳ **P2 #11** (hardcoded UI strings, `strings.xml` sudah ada tapi 0 dipakai) — **stage 1/4 selesai Batch67**, sisa 3 stage menyusul (dipecah per-file, bukan 1 batch raksasa — total 4 file target `SettingsScreen.kt`/`TrashScreen.kt`/`HomeScreen.kt` masing-masing lebih besar dari `OnboardingScreen.kt`, gabung semua dalam 1 batch akan tabrak HARD CAP 3-file). **Stage 1 (Batch67, 2 file)**: `strings.xml` — 21 string baru (`onboarding_page1..9_title/body` + `onboarding_skip`/`onboarding_get_started`/`onboarding_next`), teks VERBATIM dari Kotlin lama (0 wording diubah). `OnboardingScreen.kt` — `OnboardingPage.title`/`body` (`String`) jadi `titleRes`/`bodyRes` (`@StringRes Int`): `ONBOARDING_PAGES` adalah top-level `val` DILUAR @Composable manapun, jadi `stringResource()` (butuh `LocalContext.current` dari composition) gak bisa dipanggil di situ — resolusi dipindah ke `OnboardingPageContent` (yang emang @Composable) lewat `stringResource(page.titleRes)`/`stringResource(page.bodyRes)`. Tombol "Skip"/"Get Started"/"Next" juga dipindah (dipanggil langsung di `OnboardingScreen` composable, valid karena `bottomBar` lambda `Scaffold` juga `@Composable`). **Sengaja TIDAK dipindah**: field `emoji` (pictogram, bukan teks yang perlu lokalisasi). Verifikasi: brace/paren balanced (0/0 diff), `strings.xml` well-formed (parse `xml.etree.ElementTree` sukses, apostrophe/quote di teks di-escape `\'`/`\"` sesuai konvensi Android string resource), grep manual 0 sisa literal `page.title`/`page.body`/`"Skip"`/`"Get Started"` di file. **Catatan buat stage 2-4**: `SettingsScreen.kt`/`TrashScreen.kt`/`HomeScreen.kt` semuanya declare `package com.example.gallerycleaner` (flat, bukan `presentation.screen` walau folder fisiknya di situ — sesuai catatan "ROADMAP Fase C item 9" soal restrukturisasi package yang masih pending), jadi `R` class otomatis in-scope tanpa import tambahan, sama kayak batch ini. Belum termasuk catatan wording "photo"/"foto" generik dari Batch40 — itu nyusul di stage yang nyentuh `HomeScreen.kt`.- **Cleanup pending approval (Batch36)** — `SkeuoLiteTokens.kt` + `SkeuoModifier.kt` sekarang 100% unreferenced oleh `MaterialStyle`/tema manapun (`AMBER_RESERVE` pindah ke `NEUMORPH`), tapi TIDAK dihapus batch ini (butuh izin eksplisit user per aturan project). Ikut jadi dead-once-approved: 13 `val` warna lama di `Color.kt` (`EspressoSurface`, `EspressoOutline`, `IvoryText`, `IvoryTextSecondary`, `BrassKeep`, `BrassKeepDim`, `BrassKeepOnLight`, `CreamBg`, `CreamSurfaceRaised`, `CreamOutline`, `EspressoTextPrimary`, `EspressoTextSecondary`, `EspressoBg`) yang cuma dipakai `SkeuoLiteTokens.kt`. Juga ditemukan (bukan dari batch ini, sudah mati dari sebelumnya): `EspressoSurfaceRaised`, `OxbloodDeleteDim` — 0 referensi di manapun. Semua di-flag di sini, tunggu izin user sebelum hapus.
