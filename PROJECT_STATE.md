...
- **AUDIT GAP TRACKER (mulai Batch38)** — 20 temuan P0/P1/P2. Sumber sekarang **`AUDIT_GAP.md` di root repo** (ditanamkan permanen Batch45 — sebelumnya cuma ada sebagai upload chat sesi lama, itu sebabnya sempat jadi BLOCKER di awal sesi ini). Dikerjakan bertahap per batch (bukan 1 batch raksasa — di luar batas 3-file/batch project ini). Status:
  ...
  - 🔶 **P1 #6** (duplicate scan mahal untuk library besar) — **stage 1 selesai Batch45**: persistent hash cache (`HashCacheStore.kt` baru) — item yang id+size+dateModifiedMillis-nya belum berubah sejak scan terakhir reuse hash lama, gak dihash ulang. **Stage 2 (belum dikerjakan)**: 4 sub-gap sisa dari temuan audit #6 — incremental scanning granular, progress percentage ke UI, resume scan, cancellation yang benar-benar granular (saat ini exact-dup scan jalan otomatis di background lewat `LaunchedEffect` di `MainActivity`, gak ada `ScanState`/tombol cancel sama sekali, beda dari blur/near-dup yang sudah on-demand+cancellable pola `ScanState`). Stage 2 butuh ubah `MainActivity.kt` (state UI) + kemungkinan `MediaScanner.kt` lagi (progress callback/cooperative cancellation check) — sengaja dipisah biar stage 1 (paling berdampak, murni internal, 0 risiko UI) gak numpuk file dalam 1 batch bareng perubahan UI yang lebih berisiko.
