...
## Versi Saat Ini
v49 — Batch49 (In-app update, tahap 1/2: UpdateChecker + ApkDownloader, 3 file — belum ada UI/manifest, lihat Pending Queue)

## Belum Dikerjakan (Prioritas Berikutnya)
- **In-app update, tahap 2/2 (next batch, prioritas #1 sekarang)** — wiring `UpdateChecker`+`ApkDownloader` (Batch49) ke UI nyata + trigger install. Butuh 3 file: (1) `AndroidManifest.xml` (protected, edit parsial) — tambah `<uses-permission INTERNET>` (belum ada sama sekali di manifest saat ini) + `<uses-permission REQUEST_INSTALL_PACKAGES>` + deklarasi `<provider>` `FileProvider`; (2) `res/xml/file_paths.xml` (baru) — expose `getExternalFilesDir(null)/updates/` ke FileProvider; (3) `SettingsScreen.kt` — baris "Check for Update" (state check/downloading-progress/ready-to-install, cupertino bottom sheet buat release notes, tombol install manggil `Intent(ACTION_VIEW)` dengan `Uri` dari FileProvider + `FLAG_GRANT_READ_URI_PERMISSION`). 3 file pas HARD CAP, gak ada sisa buat batch ini.
- **AUDIT GAP TRACKER (mulai Batch38)** — 20 temuan P0/P1/P2. Sumber sekarang **`AUDIT_GAP.md` di root repo** (ditanamkan permanen Batch45 — sebelumnya cuma ada sebagai upload chat sesi lama, itu sebabnya sempat jadi BLOCKER di awal sesi ini). Dikerjakan bertahap per batch (bukan 1 batch raksasa — di luar batas 3-file/batch project ini). Status:
  ...
  - 🔶 **P1 #6** (duplicate scan mahal untuk library besar) — **stage 1 selesai Batch45**: persistent hash cache (`HashCacheStore.kt` baru) — item yang id+size+dateModifiedMillis-nya belum berubah sejak scan terakhir reuse hash lama, gak dihash ulang. **Stage 2a selesai Batch47** (`MediaScanner.kt` + `MediaRepository.kt`, 2 file): loop hashing sebelumnya 0 suspension point walau `suspend fun` — Job cancel (mis. `LaunchedEffect(activeMedia)` restart) gak pernah beneran berhenti, scan lama tetap jalan sampai selesai sia-sia. Fix: `yield()` tiap 20 item (cadence sama `findBlurryPhotos`) bikin cancel beneran jalan, + `onProgress(checked,total)` callback opsional (default no-op, 0 call-site lama berubah). Cache disimpan di `finally` — scan yang dibatalkan tetap nyimpen hash yang sempat kehitung, scan berikutnya lebih murah lewat cache stage 1 (bukan resume posisional, sekadar byproduct jujur dari arsitektur cache, didokumentasikan apa adanya bukan diklaim lebih). **Stage 2b (belum dikerjakan, next)**: tombol Scan on-demand + Cancel + progress% di UI, pola `ScanState` sama persis kayak blur/near-dup — butuh `MainActivity.kt` (state + fungsi trigger, ganti auto-scan `LaunchedEffect` jadi manual) + `HomeScreen.kt` (UI card, mirip row blurry/near-dup). Sengaja dipisah dari 2a: gabung 4 file (MediaScanner+MediaRepository+MainActivity+HomeScreen) dalam 1 batch akan tabrak HARD CAP 3-file, dan UI/state change lebih berisiko tanpa compiler nyata — dites terpisah biar kalau ada masalah, gampang dilacak batch mana penyebabnya.
  - ⏳ P1 #7-10, P2 #11-20 — belum disentuh, menyusul (urutan sesuai "PRIORITAS FIX" di `AUDIT_GAP.md`). **Catatan dari Batch40**: P2 #11 (hardcoded strings) sekarang juga mencakup kata "photo"/"foto" yang dipakai generik untuk semua item termasuk video di beberapa layar (mis. hitungan "X photos" di Home) — belum diubah, dicatat biar gak kelupaan saat P2 #11 dikerjakan. **Catatan dari Batch42**: kalau OS-level trash (`MediaStore.createTrashRequest()`) jadi requirement produk nyata di masa depan, itu batch tersendiri (lihat detail Batch42 di Riwayat Batch), bukan bagian dari P1/P2 manapun.- **Cleanup pending approval (Batch36)** — `SkeuoLiteTokens.kt` + `SkeuoModifier.kt` sekarang 100% unreferenced oleh `MaterialStyle`/tema manapun (`AMBER_RESERVE` pindah ke `NEUMORPH`), tapi TIDAK dihapus batch ini (butuh izin eksplisit user per aturan project). Ikut jadi dead-once-approved: 13 `val` warna lama di `Color.kt` (`EspressoSurface`, `EspressoOutline`, `IvoryText`, `IvoryTextSecondary`, `BrassKeep`, `BrassKeepDim`, `BrassKeepOnLight`, `CreamBg`, `CreamSurfaceRaised`, `CreamOutline`, `EspressoTextPrimary`, `EspressoTextSecondary`, `EspressoBg`) yang cuma dipakai `SkeuoLiteTokens.kt`. Juga ditemukan (bukan dari batch ini, sudah mati dari sebelumnya): `EspressoSurfaceRaised`, `OxbloodDeleteDim` — 0 referensi di manapun. Semua di-flag di sini, tunggu izin user sebelum hapus.
